...
class AdaBoost:
    ...
    def stump_error(self, y, y_pred, sample_weights):
        ...
        # TODO
        WronglyClassified=np.where(y_pred!=y,1,0)
        err=np.sum(sample_weights*WronglyClassified)/np.sum(sample_weights)
        return err
    ...
    def predict(self, X):
        ...
        for i in range(self.n_stumps):
            results.append(np.sign(self.stumps[i].predict(X)))
        for i in range(self.n_stumps):
