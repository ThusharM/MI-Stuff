...
class Tensor:
	...
	def grad_add(self, gradients=None):
		...
		opOne = self.history[1]
		opTwo = self.history[2]


		opOne.grad=np.zeros_like(opOne.arr)
		opTwo.grad=np.zeros_like(opTwo.arr)


		if opOne.requires_grad: 
			opOne.grad += np.ones_like(opOne.arr)


		if opTwo.requires_grad: 
			opTwo.grad += np.ones_like(opTwo.arr)


		if gradients is None:
			return (opOne.grad,opTwo.grad)


		if opOne.requires_grad: 
			opOne.grad = np.multiply(np.ones_like(opOne.arr), gradients)


		if opTwo.requires_grad: 
			opTwo.grad = np.multiply(np.ones_like(opTwo.arr), gradients)
	...
	def grad_matmul(self, gradients=None):
		...
		if gradients is None:
			if opOne.requires_grad:
				opOne.grad += np.matmul(np.ones_like(opOne.arr), opTwo.arr.transpose())


			if opTwo.requires_grad:
				opTwo.grad += (np.matmul(np.ones_like(opTwo.arr), opOne.arr)).transpose()


		else:
			if opOne.requires_grad:
				opOne.grad += np.multiply(np.matmul(np.ones_like(opOne.arr), opTwo.arr.transpose()), gradients)


			if opTwo.requires_grad:
				opTwo.grad += np.multiply(np.matmul(np.ones_like(opTwo.arr), opOne.arr).transpose(), gradients)
